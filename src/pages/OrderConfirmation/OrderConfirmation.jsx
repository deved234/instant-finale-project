...
const OrderConfirmation = () => {
  ...
  const shipLines = ship
    ? [
        ship.street,
        `${ship.city}${ship.zipCode ? `, ${ship.zipCode}` : ""}`,
        ship.country,
      ].filter(Boolean)
    : user?.address
      ? [
      ...
      <div className="max-w-2xl mx-auto px-6 py-12 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-gray-100 dark:border-slate-800">
          ...
          <div className="grid gap-4 sm:grid-cols-[1fr_auto] items-center">
            <div className="rounded-3xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                Your order is on its way.
              </p>
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Keep track of delivery progress and access your receipt for
                order confirmation.
              </p>
            </div>
          </div>
